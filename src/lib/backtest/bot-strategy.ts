...
function calcBodyAvg(bars: Bar[], period = 20): number[] {
...
// ── SMC detectors (exact ports from bot.py) ───────────────────

function findFVG(window: Bar[], direction: 'bullish' | 'bearish') {
...
function findIFVG(window: Bar[], price: number, direction: 'bullish' | 'bearish'): boolean {
...
function findOrderBlock(window: Bar[], direction: 'bullish' | 'bearish') {
...
function findBreaker(window: Bar[], price: number, direction: 'bullish' | 'bearish'): boolean {
  const opp = direction === 'bullish' ? 'bearish' : 'bullish'
...
function detectBOS(window: Bar[], price: number, direction: 'bullish' | 'bearish'): boolean {
...
function checkOTE(swLow: number, swHigh: number, price: number, direction: 'bullish' | 'bearish'): boolean {
...
function checkEquilibrium(rh: number, rl: number, price: number, tol = 0.006): boolean {
  const eq = (rh + rl) / 2
  return Math.abs(price - eq) / eq <= tol
}

function getStructureStop(window: Bar[], entry: number, bias: 'bullish' | 'bearish'): number {
...
// ── Daily bias engine (adapted from 1H swing → 1D swing) ──────
...
// ── Liquidity raid (same as bot.py, adapted to 15m) ───────────

function checkLiquidityRaid(
  bars: Bar[], i: number,
  bias: 'bullish' | 'bearish',
  rh: number, rl: number
): boolean {
